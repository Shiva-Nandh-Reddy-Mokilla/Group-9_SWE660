...
int lcd_data(int a, float ph, int t)
{
    char buffer[20];
    ...
    snprintf(buffer, sizeof(buffer), "Air: %d   pH: %.2f", a, ph);
	lcd_mvwrite(lcd, 0, 0, "Fish tank Monitoring");
	lcd_mvwrite(lcd, 0, 1, buffer);
    if(ph > 6.5 && ph < 7.5)
    {
        lcd_mvwrite(lcd, 0, 2, "Water Quality: Good");
    }
    else if (ph > 7.5)
        lcd_mvwrite(lcd, 0, 2, "Water Quality: Acidic");
    else if (ph < 6.5)
        lcd_mvwrite(lcd, 0, 2, "Water Quality: Basic");

    snprintf(buffer, sizeof(buffer), "Time to feed: %d", t);
	lcd_mvwrite(lcd, 0, 3, buffer);
	return 0;
}